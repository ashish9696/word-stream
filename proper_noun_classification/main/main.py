...
x_train, y_train, seq_lens_train, vocab_dict     = data_utils.load_data_and_labels(train_filename)
...
x_dev, y_dev, seq_lens_dev, vocab_dict     = data_utils.load_data_and_labels(validate_filename, train=False)
...
def dev_eval(dev_data, current_step, writer=None):
    ...
    for dev_batch in batches:
        ...
        loss_val_batch, accuracy_val_batch = dev_step(x_batch, y_batch, seq_lens_batch,             current_step, writer=writer)
        loss_val += loss_val_batch
        accuracy_val += accuracy_val_batch

    time_str = datetime.datetime.now().isoformat()
    print("Dev:   {}: step {}, loss {:g}, acc {:g}".format(time_str, current_step,         loss_val/batch_count, accuracy_val/batch_count))
...
train_summary_op, dev_summary_op, train_summary_writer, dev_summary_writer, timestamp, checkpoint_prefix =     tf_helpers.save_summaries(sess, nn.loss, nn.accuracy, grads_and_vars)
...
def run_for_epochs(batches):
    for batch in batches:
        ...
        if current_step % 100 == 0:
            train_loss, train_accuracy = train_step(x_batch, y_batch, seq_lens_batch,                 current_step, print_bool=True)
            dev_loss, dev_accuracy = dev_eval(dev_data, current_step, writer=dev_summary_writer)

        else:
            train_loss, train_accuracy = train_step(x_batch, y_batch, seq_lens_batch,                 current_step, print_bool=False)
