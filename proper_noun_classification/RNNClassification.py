...
class RNNClassifier(object):
    ...
    def __init__(self, num_classes, vocab_size, hidden_size=128, \
        ...
        masked_x = tf.multiply(self.input_x, mask_x)
        ...
        with tf.variable_scope("prediction"):
            ...
            likelihoods = tf.reduce_sum(tf.multiply(prediction_probs, one_hot_y), 1)
            log_likelihoods = tf.log(likelihoods)

            # Negative log-likelihood loss
            self.loss = tf.multiply(tf.reduce_sum(log_likelihoods), -1)
