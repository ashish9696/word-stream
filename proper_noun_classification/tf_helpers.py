...
def save_summaries(sess, loss, accuracy, grads_and_vars=None, FLAGS=None, vocab_processor=None):
	...
	# Train Summaries
	train_summary_op = tf.summary.merge([loss_summary, acc_summary, grad_summaries_merged]) if grads_and_vars else tf.summary.merge([loss_summary, acc_summary])
	train_summary_dir = os.path.join(out_dir, "summaries", "train")
	train_summary_writer = tf.summary.FileWriter(logdir=train_summary_dir, graph=sess.graph)

	# Dev summaries
	dev_summary_op = tf.summary.merge([loss_summary, acc_summary])
	dev_summary_dir = os.path.join(out_dir, "summaries", "dev")
	dev_summary_writer = tf.summary.FileWriter(logdir=dev_summary_dir, graph=sess.graph)
